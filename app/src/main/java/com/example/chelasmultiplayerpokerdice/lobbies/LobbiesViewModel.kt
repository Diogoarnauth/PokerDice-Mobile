...
class LobbiesViewModel(private val repository: LobbiesRepository) : ViewModel() {
    ...
    val state: StateFlow<LobbiesScreenState> = repository.lobbies
        .map { lobbiesInfo ->
            LobbiesScreenState.Success(lobbiesInfo) as LobbiesScreenState
        }
        .catch { error ->
            emit(LobbiesScreenState.Error(error))
        }
        .stateIn(
...
class LobbiesViewModelFactory(private val repository: LobbiesRepository) :
